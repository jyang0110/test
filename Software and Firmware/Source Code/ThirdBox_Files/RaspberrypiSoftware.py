#Group C&A
#Lab one code running at the Raspberry Pi
#Sources:
...
def read_temp():
    ...
    if Engine_check is True and checkdata == 'YES' and (GPIO.input(24) == 0):
        ...
        temp_d = temp_d / 1000
        #Negative temperatures appear as a binary number in twos complement on the seven bit display
        if temp_d < 0:
            temp_b = int(temp_d)
            temp_b = ~temp_b
            temp_b += 1
        ...
        with open("/home/pi/savedData.txt", "r+") as myfile:
            ...
            myfile.write(res + '\n' + content)
            myfile.close()
        #Update the last temperature data to the datafile.txt
